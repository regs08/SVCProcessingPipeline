...
def verify_demo_data(spectra_dir: str | Path, manifest_path: str | Path) -> Path:
    """Verify an explicitly supplied external-data manifest.

    This remains available for older cloned notebooks. The pip-first tutorial
    does not download, package, or require the external field dataset.
    """
    ...
    for item in manifest["files"]:
        ...
        if path.stat().st_size != int(item["size_bytes"]) or digest != item["sha256"]:
            mismatched.append(item["name"])

    if missing:
        raise FileNotFoundError(
            "External demo .sig data are missing. Missing files: "
            + ", ".join(missing[:5])
            + (" ..." if len(missing) > 5 else "")
        )
    if mismatched:
        raise RuntimeError(
            "External demo .sig data failed manifest verification. Mismatches: "
            + ", ".join(mismatched[:5])
    ...
    def prepare(self, verbose: bool = False) -> "PipelineConfig":
        ...
        if not sig_files:
            raise FileNotFoundError(
                ...
                "Set DATA_FOLDER in the notebook settings cell to the folder "
                "holding your raw .sig files."
            )
